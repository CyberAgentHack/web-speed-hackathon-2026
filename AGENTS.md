...
## ⚠️ AI エージェントへの最重要注意事項 — 変更前に必ず読むこと

### WSH の本質は「アプリを壊さないこと」である

WSH はパフォーマンス最適化の競技だが、**レギュレーション違反で即失格**になる。
スコアがどれだけ高くても、失格になれば全て無意味である。
過去の参加者の 9 割が「いかにアプリケーションを壊さないかを競う競技」と認識している。

**最適化の前に、挙動の維持が最優先である。**

### 変更前に必ず自問すること

変更を加える前に、以下を順番に確認すること:

1. **この変更は、ユーザーから見える挙動を変えるか？**
   - 変えるなら、`docs/test_cases.md` に明示的に許容されているか確認する
   - 明示がなければ、変更しない
2. **この変更は、テストを「回避」していないか？**
   - テストを通すために挙動を偽装するコードは**即失格**
   - テストが落ちるなら、根本原因を修正する
3. **この変更は、レギュレーションに触れないか？**
   - `docs/regulation.md` を確認する
   - 疑わしければ、変更しない・ユーザーに確認する

### レギュレーションチェックの仕組みを理解する

運営は「E2E テストで自動チェック → 失敗したものを**目視で確認**」する。
つまり:

- E2E テストが落ちても、**実際の挙動が正しければ** 許容される場合がある
- E2E テストが通っても、**目視で挙動がおかしければ** 失格になる
- **挙動 > テスト** の順で優先される
...
| NG パターン | 理由 |
|------------|------|
| 「テストに関係ないから変えていい」と判断する | 目視チェックで落ちる可能性がある |
| `sleep` / `delay` を「無駄な遅延」として除去する | 意図的な仕様の場合がある (`crok.ts` の `sleep(3000)` など) |
| ローディング表示・エラー表示を「最適化」として削除する | ユーザー体験の一部であり、目視チェック対象になりうる |
| 「明示的に禁止されていないからやる」と判断する | 禁止列挙は例示であり、網羅ではない |
| レギュレーション上グレーな変更を勝手に判断して実行する | ユーザーに確認するか、変更しない |

### 判断に迷ったときのルール

> **「やらない」が最も安全な選択肢である。**
> 効果が不確かな最適化より、失格リスクのない現状維持を選ぶ。
> グレーな変更はユーザーに相談してから実行する。
