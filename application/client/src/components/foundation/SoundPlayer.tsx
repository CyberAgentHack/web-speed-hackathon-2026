...
export const SoundPlayer = ({ sound }: Props) => {
  const { isVisible, targetRef } = useVisibility<HTMLDivElement>({
    once: true,
    rootMargin: "256px",
  });
  const { data, isLoading } = useFetch(isVisible ? getSoundPath(sound.id) : null, fetchBinary);
  ...
  return (
    <div ref={targetRef} className="bg-cax-surface-subtle flex h-full w-full items-center justify-center">
      {isLoading || data === null || blobUrl === null ? (
        <div className="flex h-full w-full items-center gap-3 px-3 text-cax-text-muted">
          <div className="bg-cax-surface-raised flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm">
          ...
          <div className="min-w-0 grow">
            ...
            <p className="overflow-hidden text-sm text-ellipsis whitespace-nowrap">{sound.artist}</p>
          </div>
        </div>
      ) : (
        <>
