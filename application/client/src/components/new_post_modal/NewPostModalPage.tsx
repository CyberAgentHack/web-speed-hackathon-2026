...
export const NewPostModalPage = ({ id, hasError, isLoading, onResetError, onSubmit }: Props) => {
  ...
  const handleChangeSound = useCallback<ChangeEventHandler<HTMLInputElement>>((ev) => {
    ...
    if (isValid) {
      ...
      import("@web-speed-hackathon-2026/client/src/utils/convert_sound").then((m) => m.convertSound(file, { extension: "mp3" })).then((converted) => {
        setParams((params) => ({
          ...params,
          images: [],
          movie: undefined,
  ...
  const handleChangeMovie = useCallback<ChangeEventHandler<HTMLInputElement>>((ev) => {
    ...
    if (isValid) {
      ...
      import("@web-speed-hackathon-2026/client/src/utils/convert_movie").then((m) => m.convertMovie(file, { extension: "mp4", size: undefined }))
        .then((converted) => {
          setParams((params) => ({
            ...params,
            images: [],
