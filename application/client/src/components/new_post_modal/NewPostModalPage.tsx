...
export const NewPostModalPage = ({ id, hasError, isLoading, onResetError, onSubmit }: Props) => {
  ...
  const handleChangeImages = useCallback<ChangeEventHandler<HTMLInputElement>>((ev) => {
    ...
    if (isValid) {
      ...
      import("@web-speed-hackathon-2026/client/src/utils/convert_image").then(({ convertImage }) => {
        const { MagickFormat } = require("@imagemagick/magick-wasm") as typeof import("@imagemagick/magick-wasm");
        return Promise.all(
          files.map((file) =>
  ...
  const handleChangeSound = useCallback<ChangeEventHandler<HTMLInputElement>>((ev) => {
    ...
    if (isValid) {
      ...
      import("@web-speed-hackathon-2026/client/src/utils/convert_sound").then(({ convertSound }) => convertSound(file, { extension: "mp3" })).then((converted) => {
        setParams((params) => ({
          ...params,
          images: [],
          movie: undefined,
