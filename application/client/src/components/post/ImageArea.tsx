...
export const ImageArea = ({ images, loading }: Props) => {
  return (
      ...
      <div className="border-cax-border grid h-full w-full grid-cols-2 grid-rows-2 gap-1 overflow-hidden rounded-lg border">
        {images.map((image, idx) => {
          return (
            ...
            >
              <CoveredImage alt={image.alt} fetchPriority={loading === "eager" && idx === 0 ? "high" : undefined} loading={loading} src={getImagePath(image.id)} />
            </div>
          );
        })}
      </div>
