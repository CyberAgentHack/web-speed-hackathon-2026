...
export const TimelineItem = ({ loading, post }: Props) => {
	const navigate = useNavigate();

	const handleClick = useCallback<MouseEventHandler>(
		(ev) => {
			let el = ev.target as Node | null;
			while (el instanceof Element && el !== ev.currentTarget) {
				if (el instanceof HTMLAnchorElement || el instanceof HTMLButtonElement || el instanceof HTMLVideoElement || el instanceof HTMLAudioElement) {
					return;
				}
				el = el.parentNode;
			}
	...
	return (
		<article className="cursor-pointer hover:bg-cax-surface-subtle px-1 sm:px-4" onClick={handleClick}>
