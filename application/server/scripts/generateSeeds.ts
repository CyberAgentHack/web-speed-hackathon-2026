...
function getImageAlt(imageDir: string, id: string): string {
  try {
    const filePath = path.join(publicDir, imageDir, `${id}.jpg`);
    const buffer = readFileSync(filePath);
    return extractExifDescription(buffer);
  } catch {
    return "";
  }
}
...
function generateProfileImages(): ProfileImageSeed[] {
  return EXISTING_PROFILE_IMAGE_IDS.map((id) => ({
    id,
    alt: getImageAlt("images/profiles", id),
  }));
}
