...
function extractIfdDescription(
  buffer: Buffer,
  tiffOffset: number,
...
function extractExifDescription(buffer: Buffer): string {
  try {
    ...
    // Check for TIFF format (starts with byte order mark)
    const firstTwo = buffer.readUInt16BE(0);
    if (firstTwo === 0x4949 || firstTwo === 0x4d4d) {
      const isLittleEndian = firstTwo === 0x4949;
      return extractIfdDescription(buffer, 0, isLittleEndian);
    }

    // JPEG format: scan for EXIF marker
    ...
    while (offset < buffer.length - 2) {
      ...
      if (marker === EXIF_MARKER) {
        ...
        if (exifHeader === "Exif\0\0") {
          ...
          return extractIfdDescription(buffer, tiffOffset, isLittleEndian);
        }
        offset += 2 + segLen;
