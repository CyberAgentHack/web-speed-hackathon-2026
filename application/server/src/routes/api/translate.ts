...
translateRouter.get("/translate", async (req, res) => {
  ...
  if (cached != null) {
    return res.status(200).type("application/json").send({ result: cached });
  }

  try {
    const result = await translate(text, { from: "ja", to: "en" });
    TRANSLATION_CACHE.set(text, result.text);
    return res.status(200).type("application/json").send({ result: result.text });
  } catch {
    return res.status(500).type("application/json").send({ error: "translation failed" });
  }
});
