...
export async function calculatePostVideoPage({ baseUrl, playwrightPage, puppeteerPage }: Params) {
  const flow = await startFlow(puppeteerPage);

  consola.debug("PostVideoPage - navigate");
  ...
  try {
    await goTo({
      ...
      puppeteerPage,
      timeout: 120 * 1000,
      url: new URL("/posts/fff790f5-99ea-432f-8f79-21d3d49efd1a", baseUrl).href,
    });
  } catch (err) {
  ...
  consola.debug("PostVideoPage - navigate end");
  const {
    steps: [result],
